...
item *gnom_item_alloc(char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes) {
    ...
    it = gnom_do_item_alloc(key, nkey, flags, exptime, nbytes, 0);
    return it;
}
...
// GNoM
void gnom_item_remove(item *item) {
    // Removing locks, a GNoM SET request already has a global lock at this point, all others are moot
#if 0
    uint32_t hv;
    hv = hash(ITEM_key(item), item->nkey, 0);
    item_lock(hv);
...
enum store_item_type gnom_store_item(item *item, int comm, conn* c) {
    ...
    hv = hash(ITEM_key(item), item->nkey, 0);
    item_lock(hv);
    ret = gnom_do_store_item(item, comm, c, hv);
    item_unlock(hv);
    return ret;
}
