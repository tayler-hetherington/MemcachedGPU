...
pthread_t cpu_only_baseline;
pthread_t gpu_network_poll; 	// Polls NIC, feeds PTF
pthread_t gpu_response_poll;	// Polls PTF, feeds NIC

CUDAContext *main_cuda_context;
/**************************************/
...
static void complete_nread_ascii(conn *c) {
    ...
    if (strncmp(ITEM_data(it) + it->nbytes - 2, "\r\n", 2) != 0) {
        ...
        printf("GNoM: complete_nread_ascii pre gnom_store_item\n");
#endif

      ret = gnom_store_item(it, comm, c);
...
static void process_bin_sasl_auth(conn *c) {
    ...
    // GNoM - Tayler: Currently not supporting Binary protocol
    assert(0);
...
// GNoM - Tayler: This function stores the item in MemcachedGPU. The GPU hash table is
// updated. If the SET request hits in the hash table, the existing entry is removed and the
// new one is stored. 
enum store_item_type gnom_do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
    
    enum store_item_type stored = NOT_STORED;

#ifdef GNOM_SET_DEBUG
        char *key = ITEM_key(it);
        printf("GNoM: gnom_do_store_item: key=%s\n", key);
#endif

    if (stored == NOT_STORED) {
        gnom_do_item_link(it, hv);
    ...
    if (stored == STORED) {
 #ifdef GNOM_SET_DEBUG
        printf("GNoM: item store success!\n");
#endif      
        c->cas = ITEM_get_cas(it);
    }
...
static void gnom_process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    char *key;
    size_t nkey;
    unsigned int flags;
    ...
    // GNoM - Tayler: This is the main function used in MemcachedGPU for item alloc
    //it = item_alloc(key, nkey, flags, realtime(exptime), vlen);
#ifdef GNOM_SET_DEBUG
        printf("GNoM: Pre gnom_item_alloc(...)\n");
#endif
    it = gnom_item_alloc(key, nkey, flags, realtime(exptime), vlen);

    if (it == 0) {
        if (! item_size_ok(nkey, flags, vlen))
            out_string(c, "GNoM: SERVER_ERROR object too large for cache");
        else
            out_string(c, "GNoM: SERVER_ERROR out of memory storing object");
        /* swallow the data line */
        c->write_and_go = conn_swallow;
        c->sbytes = vlen;
...
#ifdef GNOM_SET_DEBUG
        printf("GNoM: process_update_command - Item successfully allocated\n");
#endif
    }
...
#ifdef GNOM_SET_DEBUG
        printf("GNoM: SET request\n");
#endif
        gnom_process_update_command(c, tokens, ntokens, comm, false);

    } else if ((ntokens == 7 || ntokens == 8) && (strcmp(tokens[COMMAND_TOKEN].value, "cas") == 0 && (comm = NREAD_CAS))) {
        // GNoM - Tayler: Currently don't support CAS
        assert(0);
...
static enum try_read_result try_read_udp(conn *c) {
   ...
   // printf("Thread (%d) trying to read socket (%d)\n", syscall(SYS_gettid), ++recv_count);
    c->request_addr_size = sizeof(c->request_addr);
...
//    	}
        ...
        if ((char)buf[4] != 0 || (char)buf[5] != 1) {
      //  	printf("SERVER_ERROR reading buf[4]/[5] -> buf[4]: %c, buf[5]: %c\n", buf[4], buf[5]);
...
static enum transmit_result transmit(conn *c) {
    ...
    if (c->msgcurr < c->msgused) {
       ...
       // printf("Sendcount: %d\n", ++send_count);
    	...
    	/*
    	if(IS_UDP(c->transport)){
			//if(__sync_fetch_and_add(&g_packet_count, 1) >= 2046){
    		if(++g_packet_count >= 2047){
				g_end = RDTSC();
				printf("Total time: %lf\n", (g_end - g_start)/TSC_MULT);
				abort();
			}
    	}
    	*/
...
static void maximize_sndbuf(const int sfd) {
    ...
    int n=256*1024*1024;
    if (setsockopt(sfd, SOL_SOCKET, SO_RCVBUF, (void *)&n, sizeof(n)) == -1) {
...
static int server_socket(const char *interface,
    ...
    printf("successfully created a connection on port %d\n", port);
...
int main (int argc, char **argv) {
    ...
    // 0 = Main GPUDirect
    // 1 = Non-GPUDirect
    int gnom_config_to_run = 0;

    int gnom_device = 0; // Default = 0
    /******************************************/
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        switch (c) {
        case 'e':
            gnom_config_to_run = atoi(optarg);
            break;
        case 'g':
            gnom_device = atoi(optarg);
            break;
    ...
    /******************* GNoM **********************/
    printf("GNoM Config: ");
    switch(gnom_config_to_run){
        case 0:
            printf("Main GPUDirect\n");
            break;
        case 1:
            printf("Non-GPUDirect\n");
            break;
        default:
            printf("Error: Unrecognized GNoM config\n");
            abort();
    }


    if(gnom_device < 2) {
        printf("Using GNoM Device: %d\n", gnom_device);
    }else {
        printf("Error: Invalid GNoM Device number: %d\n", gnom_device);
        abort();
    }

    // Create CUDA context, initialize streams
    int num_streams = NUM_C_KERNELS;
    init_GPU(&main_cuda_context, num_streams, gnom_config_to_run, gnom_device);

    printf("Init_GPU complete...\n");
    // Initialize GPU resident Memcached structures (Hashtable + locks)
    gpu_mem_init(main_cuda_context, get_hashtable_size(), get_hashlock_size());

    printf("GPU_mem_init complete...\n");

    // Initialize GPU stream manager for GPU-NoM configuration
    init_stream_manager_gpu_NoM(main_cuda_context, get_GPU_hashtable(), get_GPU_lock(), gnom_config_to_run);

    printf("Init_stream_manager_GPU_NoM complete...\n");
    ...
    //settings.num_threads_per_udp = 4;
    settings.num_threads_per_udp = 0;
    printf("Actual number of UDP threads = %d (Settings = %d)\n", settings.num_threads_per_udp, settings.num_threads);
    ...
    // GNoM still needs privileges for PF_RING
    ...
    stats_init();
    assoc_init(main_cuda_context, settings.hashpower_init);
    conn_init();
