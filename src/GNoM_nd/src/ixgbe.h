...
// GNoM
#define GNOM_DEFAULT_RXD             4096 //8192
...
struct ixgbe_tx_buffer {
    ...
    unsigned int gpu_page_offset;
    int gpu_batch_id;
    int gpu_buffer_id; // ID of GPU Buffer in GPU memory
    uint64_t gpu_buffer_host_addr; // Set to the virtual address of the GPU buffer returned by CUDA malloc
    ktime_t sent_time;
    unsigned long long sanity_test_val;
};
...
struct ixgbe_rx_buffer {
	...
	// GNoM -- Needed for GPU regardless of PACKET_SPLIT for PF_RING
//#ifndef CONFIG_IXGBE_DISABLE_PACKET_SPLIT
	struct page *page;
...
/**** GNOM ****/
/*** Previously static IXGBE functions requiring prototypes here ***/
void ixgbe_tx_olinfo_status(union ixgbe_adv_tx_desc *tx_desc, u32 tx_flags, unsigned int paylen);
