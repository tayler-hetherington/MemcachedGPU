...
// Removed the driver from the PF_RING subdirectory to merge with GNoM. 
//#include "../../../../../../kernel/linux/pf_ring.h"
#include "pf_ring.h" // NOTE: This requires that PF_RING_DIR is set to the PF_RING installation directory. 
...
#ifdef DO_GNOM_TX
        // Testing initial GNoM TX
		if(tx_buffer->gpu_buffer_id != 123){
#endif
            /* free the skb */
            dev_kfree_skb_any(tx_buffer->skb);
	        ...
	        printk("[GNoM_ixgbe] Packet 123 clean: ring: %p, tx_buffer: %p, tx_desc: %p, next_to_use: %d, next_to_clean: %d, next_to_watch: %p\n", tx_ring, tx_buffer, tx_desc, tx_ring->next_to_use, tx_ring->next_to_clean, tx_buffer->next_to_watch);
		}
		tx_buffer->gpu_buffer_id = 0;
#endif
...
#ifdef CONFIG_IXGBE_DISABLE_PACKET_SPLIT
		// GNoM
	    if(likely(rx_ring->is_gpu_ring)){ // Allocate buffer from GPU free list
			if(unlikely(!ixgbe_alloc_gpu_mapped_page(rx_ring, bi))){
				gnom_inc_rx_map_failed(1);
				goto gnom_no_free_buf;
			}else{
				gnom_inc_rx_alloc(1);
			}
		}else{
...
#else
		if(likely(is_gnom_rx_enabled() && rx_ring->is_gpu_ring)){ // Allocate buffer from GPU free list
			if(!ixgbe_alloc_gpu_mapped_page(rx_ring, bi)){
			    gnom_inc_rx_map_failed(1);
				break;
			}else{
			    gnom_inc_rx_alloc(1);
...
//			break;
		...
		// GNoM
#ifdef CONFIG_IXGBE_DISABLE_PACKET_SPLIT
		// GPU requires packets to be split, update with dma_addr + offset
        if(likely(rx_ring->is_gpu_ring))
			       ...
			       int budget)
{
	...
	do {
		...
		//printk("\t\t[GNOM_IXGBE]: reg_idx: %d, rx_ring: %p, rx_desc: %p, skb: %p, ntc: %d, nta: %d\n", rx_ring->reg_idx, rx_ring, rx_desc, skb, rx_ring->next_to_clean, rx_ring->next_to_alloc);
		//printk("\t\t[GNOM_IXGBE]: (ixgbe_clean_rx_irq) rx_ring->desc: %p, rx_ring->dma: %p, cleaned_count: %d\n", rx_ring->desc, rx_ring->dma, cleaned_count);
...
int ixgbe_poll(struct napi_struct *napi, int budget)
{
	...
	int GNOM_RX_BUDGET = MAX_REQ_BATCH_SIZE;
    ...
    ixgbe_for_each_ring(ring, q_vector->tx){
#ifdef DO_GNOM_TX
    	if(unlikely(!ring->is_gpu_ring)){
            clean_complete &= ixgbe_clean_tx_irq(q_vector, ring);
        }else{
			clean_complete &= ixgbe_clean_gpu_tx_irq(q_vector, ring);
        }
#else
    	clean_complete &= ixgbe_clean_tx_irq(q_vector, ring);
#endif
    ...
    ixgbe_for_each_ring(ring, q_vector->rx){
        if(likely(ring->is_gpu_ring)){
            clean_complete &= (ixgbe_clean_gpu_rx_irq(q_vector, ring, GNOM_RX_BUDGET) < GNOM_RX_BUDGET); // Changing budget to size of batch
        }else{
            clean_complete &= (ixgbe_clean_rx_irq(q_vector, ring, per_ring_budget) < per_ring_budget);
        }
    }
	...
	ixgbe_for_each_ring(ring, q_vector->rx){
		if(!ring->is_gpu_ring){
			...
			clean_complete &= (ixgbe_clean_gpu_rx_irq(q_vector, ring, per_ring_budget) < per_ring_budget);
		}
	}
	*/
				   ...
				   struct ixgbe_ring *rx_ring)
{
    ...
    //b_len =  ALIGN(rx_ring->rx_buf_len, 1024);
#else
	srrctl |= ixgbe_rx_bufsz(rx_ring) >> IXGBE_SRRCTL_BSIZEPKT_SHIFT;
    //b_len = ixgbe_rx_bufsz(rx_ring);
#endif

    //printk("[GNOM_IXGBE] buffer_len: %d\n", b_len);
...
//		       __FUNCTION__, netdev->mtu, max_frame, rx_buf_len);
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
        ...
        // GNoM
        printk("[GNOM_IXGBE] rx_buf_len set to %d\n", rx_buf_len);
#ifdef IXGBE_FCOE
...
static void ixgbe_configure_rx(struct ixgbe_adapter *adapter)
{
	...
	//printk("[GNOM_IXGBE] Adapter hw: %p\n", adapter->hw.hw_addr);
	for (i = 0; i < adapter->num_rx_queues; i++)
...
static void ixgbe_configure(struct ixgbe_adapter *adapter)
{
    ...
    if(hook){
        hook->gpu_set_device(dev);
    }
    
	/* Check if GPU module has been initialized */
	if(!strncmp(netdev->name, GNOM_RX_NAME, 4)){
		if(setup_gpu_rx_env(adapter))
			printk("[GNOM_IXGBE]: Error: setup_gpu_rx_env failed...\n");

		for(i=0;i<adapter->num_rx_queues; ++i){
			if(adapter->rx_ring[i]->is_gpu_ring)
				printk("[GNOM_IXGBE] %s RX_ring %d is a GPU ring...\n", GNOM_RX_NAME, i);
			else
				printk("[GNOM_IXGBE] %s RX_ring %d is a CPU ring...\n", GNOM_RX_NAME, i);
		}
	}
    

   	if(!strncmp(netdev->name, GNOM_TX_NAME, 4)){

#ifdef DO_GNOM_TX
        if(setup_gpu_tx_env(adapter))
            printk("[GNOM_IXGBE]: Error: setup_gpu_tx_env failed...\n");
#endif

        for(i=0;i<adapter->num_tx_queues; ++i){
            if(adapter->tx_ring[i]->is_gpu_ring)
                printk("[GNOM_IXGBE] %s TX_ring %d is a GPU ring...\n", GNOM_TX_NAME, i);
            else
                printk("[GNOM_IXGBE] %s TX_ring %d is a CPU ring...\n", GNOM_TX_NAME, i);
        }
    }
...
#else
				rx_info.packet_memory_slot_len      = ALIGN(ixgbe_rx_bufsz(rx_ring), cache_line_size);
#endif
...
static int __devinit ixgbe_sw_init(struct ixgbe_adapter *adapter)
{
	...
	// GNoM
	adapter->rx_ring_count = GNOM_DEFAULT_RXD; // Overwrite
...
*/
    adapter->tx_work_limit = IXGBE_DEFAULT_TX_WORK;
    adapter->rx_work_limit = MAX_REQ_BATCH_SIZE; // 512 req/batch
...
int ixgbe_setup_tx_resources(struct ixgbe_ring *tx_ring)
{
	...
	// GNoM
	// Setting default to non-gpu buffer ring
	tx_ring->is_gpu_ring = 0;
...
int ixgbe_setup_rx_resources(struct ixgbe_ring *rx_ring)
{
	...
	//printk("[GNOM_IXGBE] DMA_alloc_coh, rx_ring->desc: %p, rx_ring->dma: %p, ring: %d, rx_ring->count (desc): %d, size: %d\n", (void *)rx_ring->desc, (void *)rx_ring->dma, rx_ring->reg_idx, rx_ring->count, rx_ring->size);
	...
	// GNoM Setting default to non-gpu buffer ring
	rx_ring->is_gpu_ring = 0;
...
static int ixgbe_open(struct net_device *netdev)
{
	...
	printk("\n\n======= GNoM-ND Modified IXGBE Driver -- ixgbe_open ========\n");
			 ...
			 const u8 hdr_len)
{
	...
	size = skb_headlen(skb); // == (skb->len - skb->data_len)
	data_len = skb->data_len;

    /*
	if( ntohs(tcp_hdr(skb)->source) == 9999 || ntohs(tcp_hdr(skb)->source) == 9998){
	    print_pkt_hdr(skb, 1);
	    print_skb_info(skb);
	}
    */
	...
	if(unlikely(tx_ring->is_gpu_ring)){
	    printk("[GNoM_ixgbe]: ERROR Trying to perform CPU TX on GNoM GPU TX Ring...\n");
	}
	tx_ring->next_to_use = i;
...
// GNoM: Allows legacy send for a subset of TX_RINGs.
//       A dedicated GPU ring handles GNoM send traffic
...
#ifdef DO_GNOM_TX
	// GNoM - Regular network traffic on tx_ring[0], GNoM on tx_ring[1]
	tx_ring = adapter->tx_ring[0];
	if(unlikely(tx_ring->is_gpu_ring)){
	    printk("[GNOM_IXGBE]: **ERROR** Regular network traffic using GPU ring\n");
	}
#else
    // Legacy
			      ...
			      netdev_features_t features)
{
	...
	case NETIF_F_NTUPLE:
		...
		printk("[GNOM_IXGBE]: Trying to enable Ntuple\n");
				 ...
				 const struct pci_device_id *ent)
{
	...
	printk(KERN_EMERG "======= GNoM-ND Modified IXGBE Driver -- ixgbe_probe ========\n");
	...
	// GNoM - Set Perfect flow director flag
    adapter->flags &= ~IXGBE_FLAG_FDIR_HASH_CAPABLE;
    adapter->flags |= IXGBE_FLAG_FDIR_PERFECT_CAPABLE;

    init_tx_lock();

	if(!strncmp(pci_name(pdev), GNOM_RX_DEVICE_PCI_NAME, 12)){
		set_gnom_rx_adapter(adapter);
		printk("[GNOM_IXGBE]: Setting gnom_rx_adapter for %s: %p\n", pci_name(pdev), adapter);
        printk("[GNOM_IXGBE]: -%s- Enabling ntuple-filter internally (note: ethtool still reports 'off' even though it's on)\n", pci_name(pdev));
	}else if(!strncmp(pci_name(pdev), GNOM_TX_DEVICE_PCI_NAME, 12)){
        set_gnom_tx_adapter(adapter);
        printk("[GNOM_IXGBE]: Setting gnom_tx_adapter for %s: %p\n", pci_name(pdev), adapter);
	}

	netdev->hook_to_gpu_mod = NULL;
